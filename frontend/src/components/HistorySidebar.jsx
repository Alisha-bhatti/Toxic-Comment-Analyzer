    ...
    <>
      ...
      {!isOpen && (
        <motion.button
          ...
          onClick={onToggle}
          className="fixed right-0 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-md border-y border-l border-white/20 p-4 rounded-l-3xl shadow-2xl z-40 group hover:bg-primary transition-all duration-300"
        >
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed right-0 top-0 h-screen w-full max-w-sm bg-white/90 backdrop-blur-2xl border-l border-white/20 shadow-2xl z-50 flex flex-col"
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {filteredHistory.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-10 space-y-4">
                    ...
                    <p className="font-bold text-slate-400">No records found matching "{filter}"</p>
                  </div>
                ) : (
                  filteredHistory.map((item) => (
                    <motion.div
                      ...
                      className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm cursor-pointer hover:border-primary/30 transition-all group"
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-slate-600 line-clamp-2 group-hover:text-slate-900 transition-colors">
                        {item.original_text}
                      </p>
                      <div className="mt-3 pt-3 border-t border-slate-50 flex items-center justify-between">
                         <div className="flex -space-x-1">
                            ...
                            {Object.entries(item.categories_json || {}).map(([key, val], idx) => (
                                val > 0.5 && <div key={key} className="w-2 h-2 rounded-full border border-white" style={{ backgroundColor: val > 0.8 ? '#EF4444' : '#F59E0B' }} />
                            ))}
                         </div>
